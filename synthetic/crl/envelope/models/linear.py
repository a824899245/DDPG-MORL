...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, reward_size):
        ...
        self.affine1 = nn.Linear(state_dim + 2 * reward_size, (state_dim + 2 * reward_size)*4).to('cuda')
        self.affine2 = nn.Linear((state_dim + 2 * reward_size)*4, (state_dim + 2 * reward_size)*8).to('cuda')
        self.affine3 = nn.Linear((state_dim + 2 * reward_size)*8, (state_dim + 2 * reward_size)*4).to('cuda')
        self.affine4 = nn.Linear((state_dim + 2 * reward_size)*4, (state_dim + 2 * reward_size)*2).to('cuda')
        self.affine5 = nn.Linear((state_dim + 2 * reward_size)*2, action_dim).to('cuda')

    def forward(self, state, preference_i, preference_):
        state = state.view(state.size(0), -1)
        x = torch.cat((state, preference_i, preference_), dim=1)
        x = x.view(x.size(0), -1)
        x = F.relu(self.affine1(x))
        x = F.relu(self.affine2(x))
        x = F.relu(self.affine3(x))
        x = F.relu(self.affine4(x))
        action = torch.tanh(self.affine5(x))
...
class Critic(nn.Module):
    def __init__(self, state_size, action_size, reward_size):
        ...
        self.affine1 = nn.Linear(state_size + reward_size + action_size,
                                 (state_size + reward_size + action_size) * 16).to('cuda')
        self.affine2 = nn.Linear((state_size + reward_size + action_size) * 16,
                                 (state_size + reward_size + action_size) * 32).to('cuda')
        self.affine3 = nn.Linear((state_size + reward_size + action_size) * 32,
                                 (state_size + reward_size + action_size) * 16).to('cuda')
        self.affine4 = nn.Linear((state_size + reward_size + action_size) * 16,
                                 (state_size + reward_size + action_size) * 8).to('cuda')
        self.affine5 = nn.Linear((state_size + reward_size + action_size) * 8,
                                 reward_size).to('cuda')

    def forward(self, state, action, preference):
        state = state.view(state.size(0), -1).to('cuda')
        action = action.view(action.size(0), -1).to('cuda')
...
class EnvelopeLinearCQN(torch.nn.Module):
    ...
    def __init__(self, state_size, action_size, reward_size):
        ...
        self.affine1 = nn.Linear(state_size + reward_size,
                                 (state_size + reward_size) * 16).to('cuda')
