...
parser.add_argument('--noise', type=float, default=0.05, metavar='noise',
                    help='(initial) noise for action selection, default = 0.05')
...
def train(env, agent, args):
    ...
    for num_eps in range(args.episode_num):
        ...
        if args.env_name == "dst":
            ...
            act_1 = agent.predict([0,0],[1,0],probe).squeeze()
            act_2 = agent.predict([0,0],[0,1],probe).squeeze()
        ...
        if args.method == "crl-naive":
            ...
            act_1 = probe.dot(act_1.data)
            act_2 = probe.dot(act_2.data)
        print("end of eps %d with total reward (1) %0.2f, the Q is %0.2f | %0.2f; c_loss: %0.4f, a_loss: %0.4f" % (
            num_eps,
            tot_reward,
            act_1,
            act_2,
...
if __name__ == '__main__':
    ...
    if args.serialize:
        ...
        model,a_model,a_model_t, c_model,c_model_t = get_new_model(args.model,
         state_size, action_size, reward_size)
    agent = MetaAgent(model,a_model,a_model_t, c_model,c_model_t, args, is_train=True)
